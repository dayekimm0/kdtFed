...
(function (global, factory) {
  ...
  var focusableElementsString =
    'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex="0"], summary:not([disabled]), [contenteditable]'; // cache common elements
  ...
  function lazyLoadDirection(startPanel, direction, count) {
    ...
    for (
      var i = 0;
      i < count && (currentPanel = currentPanel[direction]());
      i++
    ) {
  ...
  var scrollOverflowHandler = {
    ...
    preventScrollWithKeyboard: function preventScrollWithKeyboard(e) {
      if (!isInsideInput() && getOptions().keyboardScrolling) {
        ...
        if (
          directionKeys.indexOf(e.keyCode) > -1 &&
  ...
  function onSlideOrScroll(params) {
    ...
    var areConsecutivePanels =
      Math.abs(params.items.origin.index() - params.items.destination.index()) >
      1;
    var doesApply =
      (skipValue === true || skipValue === scrollType) && areConsecutivePanels;
  ...
  function displayWarnings() {
    ...
    if (getOptions().licenseKey.trim() === "") {
      ...
      console.warn(
        "%c This website was made using fullPage.js slider. Learn more on the following website:",
        msgStyle
      );
